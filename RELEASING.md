...
`github:mat3ra/cove#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/cove/releases/download/wip-<short-sha>/cove.tgz
```
...
```bash
npm run mat3ra:install -- cove wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/cove@<release-url>` with
