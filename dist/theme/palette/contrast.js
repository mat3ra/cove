...
function parseHexColor(color) {
    const hex = color.length === HEX_SHORTHAND_LENGTH
        ? color
            .slice(1)
            .split("")
...
export function compositeOver(foreground, background) {
    const blend = (foregroundChannel, backgroundChannel) => foregroundChannel * foreground.alpha + backgroundChannel * (1 - foreground.alpha);
    return {
 ...
 */
export function contrastRatio(foregroundColor, backgroundColor, surface = "#FFFFFF") {
